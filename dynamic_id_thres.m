...
datatxt=[filename '_g' subgender '_a' subage '_r' randtype '_DyIdthr.csv'];
...
% name list
% Antoine, Sophie, Didier, Fanny, Gr?goire, H?l?ne, Joseph, Katia
% Male1, Female1, Male2, Female2, Male3, Female3, Male4, Female4
ak=KbName('a');% Antoine
sk=KbName('s');% Sophie
dk=KbName('d');% Didier
fk=KbName('f');% Fanny
gk=KbName('g');% Gr?goire
hk=KbName('h');% H?l?ne
jk=KbName('j');% Joseph
kk=KbName('k');% Katia
...
while ii2<Nexpall
    ...
    if ~isnan(seedlist(seed_loc,2))
        ...
        if recorder(1)>20 % Uniform until the 20th trials
            history_resp = squeeze(Rsp_allid(idexp,1:(recorder(1)-1),:));
            % smooth
